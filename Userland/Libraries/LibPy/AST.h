...
class PyCastExpression : public Expression {
public:
    PyCastExpression(ASTNode* parent, Optional<Position> start, Optional<Position> end, const String& filename)
        : Expression(parent, start, end, filename)
    {
    }

    virtual ~PyCastExpression() override = default;
    virtual const char* class_name() const override { return "PyCastExpression"; }
    virtual void dump(FILE* = stdout, size_t indent = 0) const override;
