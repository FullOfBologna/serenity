...
{
    return match(Token::Type::ImportStatement);
}
...
void Parser::consume_preprocessor()
{
    ScopeLogger<PY_DEBUG> logger;
    switch (peek().type()) {
    case Token::Type::ImportStatement:
        consume();
        consume(Token::Type::ImportModule);
        break;
    default:
...
StringView Parser::text_of_token(const Py::Token& token) const
{
    return token.text();
}
...
NonnullRefPtr<PyCastExpression> Parser::parse_cpp_cast_expression(ASTNode& parent)
{
    auto cast_expression = create_ast_node<PyCastExpression>(parent, position(), {});
